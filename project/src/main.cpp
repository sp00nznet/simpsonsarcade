...
static void crash_log_write(const char* fmt, ...) {
    ...
    if (!g_crash_log) {
        g_crash_log = fopen("simpsons_crash.log", "a");
...
static struct StderrRedirect_ {
    StderrRedirect_() {
        freopen("simpsons_stderr.log", "w", stderr);
        fprintf(stderr, "[simpsons] stderr redirected to log file\n");
        fflush(stderr);
        g_crash_log = fopen("simpsons_crash.log", "w");
        if (g_crash_log) {
            fprintf(g_crash_log, "[simpsons] Crash log initialized\n");
            fflush(g_crash_log);
        }
        std::set_terminate([]() {
            crash_log_write("\n========== TERMINATE CALLED ==========\n");
...
class SimpsonsApp : public rex::ui::WindowedApp, public rex::ui::WindowListener {
public:
    static std::unique_ptr<rex::ui::WindowedApp> Create(rex::ui::WindowedAppContext& ctx) {
        return std::make_unique<SimpsonsApp>(ctx);
    }

    SimpsonsApp(rex::ui::WindowedAppContext& ctx)
        : WindowedApp(ctx, "simpsons", "[game_directory]") {
    ...
    bool OnInitialize() override {
        ...
        window_ = rex::ui::Window::Create(app_context(), "The Simpsons Arcade", 1280, 720);
        if (!window_) {
